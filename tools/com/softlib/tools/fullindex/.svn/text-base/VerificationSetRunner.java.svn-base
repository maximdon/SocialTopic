...
public class VerificationSetRunner 
{
	private static Logger log = Logger.getLogger(VerificationSetRunner.class);
	public VerificationSetRunner()
	{
	...
	private boolean verifyMatch(String objectId, String ticketID, Set<String> expectedResults,float minimalScore,int count)
	{
		...
		try {
			...
			ITicket ticket = ticketProvider.get(ticketID);
			if (ticket == null)
				return false;
			
			IMatcher matcher = (IMatcher)info.getBean("matcher");
				try {
					MatchResults matchResults = matcher.findMatches(ticket);
					int idx=0;
					for(MatchCandidate candidate : matchResults.getCandidates()) 
					{
						...
						{
							matchesOverScore++;
							if (expectedResults.contains(fixId(candidate.getCandidateData().getId())))
							{
								
								LogUtils.info(verifyLog,"Found: %s index=%s score=%s" , candidate.getCandidateData().getId(),idx,candidate.getScore());
					...
					if (expectedResults.size() == 0)
					{
						if (matchesOverScore == 0)
						{
							...
							LogUtils.debug(log, "For ticket %s no matches as expected", ticket);									
							return true;
						}
						else
						{
